...
if __name__ == "__main__":
    ...
    # My functions
    fueltypes = ppl.Fueltype.unique()
    #Hydro, HardCoal, CCGT, OCGT, Lignite, Nuclear, Oil, Pv, Bioenergy, CCGT, Thermal, Geothermal, Waste, Storage Technologies, Other
    d = {'Fueltype': list(fueltypes), 'factor': [1.64110069, 0.689520075, 0.513475166, 0.513475166, 0.689520075, 0.775629886, 0.850890279, 1, 1.682898103, 0.513475166, 1, 1, 1, 1]}
    factors = pd.DataFrame(data=d)
    ppl2 = ppl.merge(factors[['Fueltype', 'factor']], on='Fueltype')
    ppl2['new_Capacity'] = ppl2['Capacity'] * ppl2['factor']
    ppl2['Capacity'] = ppl2['new_Capacity']
    pl2 = ppl2.drop(['new_Capacity', 'factor'], axis=1)
    ppl = ppl2

    # duplicate all ccgt generators (to make them with ccs)
    #ccs = ppl.loc[ppl['Fueltype'] == 'CCGT']
    #ccs.loc[:,'Fueltype'] = 'CCGT, CCS'
    #ppl_ccs = ppl.append([ccs],ignore_index=True)
    #ppl = ppl_ccs
    
    #add single normal generator
    #ccs = ppl.loc[ppl['Name'] == 'Spalding']
    #ccs.Capacity = 4510.85824
    #ppl_ccs = ppl.append([ccs],ignore_index=True)
    #ppl = ppl_ccs
